...
<?php $this->load->view('admin/header'); ?>
    ...
    <div class="container-fluid" style="min-height: 76.6vh;">
        <div class="header">
            <h5 style="font-weight: bold;">Numbering</h5>
        </div>
        <div class="body">
            <div class="col-md-12">
                <form class="form form-horizontal" method="POST" action="<?php echo base_url() ?>admin/numbering_add">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <select class="form-control input-size-auto" name="document" required>
                                    <option selected="" disabled="">Please Select Document</option>
                                    <!--v-for-start--><option value="Quotation">Quotation</option><option value="Job Order">Job Order</option><option value="Delivery Order">Delivery Order</option><option value="BOQ">BOQ</option><option value="Schedule">Schedule</option><option value="DKM">DKM</option><option value="MPK">MPK</option><option value="Packing List">Packing List</option><option value="Purchase Request">Purchase Request</option><option value="Purchase Order">Purchase Order</option><option value="SPB">SPB</option><option value="BSK">BSK</option><option value="Penerimaan Consumable">Penerimaan Consumable</option><option value="Pengambilan Consumable">Pengambilan Consumable</option><option value="Inspection and Test Plan">Inspection and Test Plan</option><option value="Transaction">Transaction</option><option value="Request Consumable">Request Consumable</option><!--v-for-end-->
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-md-3">Periodic</label>
                            <div class="col-md-9">
                                <select class="form-control input-size-auto" name="periodic">
                                    <option value="monthly">Monthly</option>
                                    <option value="yearly">Yearly</option>
                                    <option value="no-periodic">Not Periodic</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-md-3">Pattern</label>
                            <div class="col-md-9">
                                <input type="text" class="form-control input-sm input-size-md" name="pattern">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-md-3">Last Number</label>
                            <div class="col-md-3">
                                <input type="text" class="form-control input-sm" name="last_number">
                            </div>
                            <div class="col-md-6">
                                <i>without leading zero(s).</i>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <table class="table table-condensed table-hover table-bordered datatable" style="margin-top: 0;">
                            <thead>
                            <tr>
                                <th colspan="3">Available pattern variable.</th>
                            </tr>
                            <tr>
                                <th>Variable</th>
                                <th>Description</th>
                        ...
                        <tbody>
                            <?php $i=1; foreach ($table as $key => $value) {?>
                                <tr>
                                    ...
                                    <td><?php echo $value->periodic; ?></td>
                                    <td><?php echo $value->last_number; ?></td>
                                    <td><center><button class="btn btn-info edit_button" data="<?php echo $value->id; ?>" document="<?php echo $value->document; ?>" pattern="<?php echo $value->pattern; ?>" periodic="<?php echo $value->periodic; ?>" last_number="<?php echo $value->last_number; ?>"><i class="fa fa-edit"></i> Edit</button><button class="btn btn-danger remove_button" data="<?php echo $value->id; ?>"><i class="fa fa-trash"></i> Delete</button></center></td>
                                </tr>
                            <?php $i+=1; } ?>
                        </tbody>
                    </table>
...
<script>
  $(document).ready(function(){
      ...
      $(document).on('click',".edit_button",function(){
        ...
        $("select[name='document']").val($(this).attr("document"));
        $("select[name='periodic']").val($(this).attr("periodic"));
        $("input[name='pattern']").val($(this).attr("pattern"));
        $("input[name='last_number']").val($(this).attr("last_number"));
      });
